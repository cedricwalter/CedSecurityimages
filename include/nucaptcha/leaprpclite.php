...
if(class_exists("lmRPCLite", false))
{
	return;
}
...
class lmRPCLite
{
     ...
     * @var string
     */
    private static $sUserAgent = 'LEAP-RPC-PHP2 1.0.10903';
    ...
    public function GetResult()
    {
		...
		{
			$server = $this->mServer;
			throw new LeapException("Could not read from server: '$server[host]'", LMEC_COULDNOTCONNECT);
		}
		...
		$headers = explode("\n", $header);
		if (!lmRPCLite::VerifyHttpResponse($headers))
		{
			$server = $this->mServer;
			throw new LeapException("Could not read from server: '$server[host]'", LMEC_COULDNOTCONNECT);
		}
    ...
    /**
     * Create a new lmRPCLite object for querying Leap servers.
     *
    ...
    public function __construct($server, $message)
    {
		...
		{
			throw new LeapException("'$server' is an invalid URL", LMEC_COULDNOTCONNECT);
		}
    ...
    {
		$client = new lmRPCLite($server, $message);
