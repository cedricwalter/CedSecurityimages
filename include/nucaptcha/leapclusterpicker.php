...
class lmClusterPicker
{
    ...
    private static function GetDNS ($clusterRecord)
    {
		...
		if(sizeof($result) == 0)
		{
			throw new LeapException('Failed DNS lookup for '. $clusterRecord . '.',	LMEC_COULDNOTCONNECT);
		}
    ...
    protected static function ParseRecord ($record)
    {
		...
		if(0 == sizeof($results))
		{
			throw new LeapException("Cluster record could not be split.", LMEC_INVALIDDATA);	
		}
		elseif('' == $results[0])
		{
			throw new LeapException("Cluster record is empty", LMEC_INVALIDDATA);	
		}
    ...
    private static function GetServer ($configName, $prefix)
    {
		/*try
		{
			$server = lmDefaultConfig::GetGlobal($configname);
