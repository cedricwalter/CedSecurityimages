...
if(class_exists("lmBase64", false))
{
	return;
}
...
class lmBase64
{
	...
	static public function EncodeArray($array)
	{
		...
		foreach($array as $key=>$value)
		{
			...
			{
				throw new LeapException("lmBase64::Encode: unknown format ".gettype($message), LMEC_INVALIDDATA);
			}
		}
		//print("\narray: $data\n");
	...
	static private function ReadIntFromString($data, &$pos)
	{
		...
		if (false === $newpos)
		{
			throw new LeapException("DecodeArray::ReadIntFromString - no int found", LMEC_INVALIDDATA);
		}
		...
		while ($num-- > 0)
		{
			...
			switch ($type)
			{
				...
				case 's':
					...
					break;
				default:
					throw new LeapException("lmBase64::Unknown format $type in the array", LMEC_INVALIDDATA);
					break;
			}
		}
