...
class lmResponse
{
    ...
    public function __construct($input)
    {
		if( $input != null )
		{
			...
			$chunkData = lmTextChunk::Decode($input, "PDATA");
			...
			{
				throw new LeapException("Invalid response data for FIELDS", LMEC_INVALIDDATA);
			}

			if( $this->mAnswerName == null || $this->mAnswerName === false )
			{
				throw new LeapException('Unable to find RESP chunk in FIELDS', LMEC_INVALIDDATA);
			}
		}
    }
     ...
     *
     * @param lmTextChunk $chunk - the chunk to fill in
     */
    public function SetValidationFields(&$chunk)
    {
		...
		foreach ($this->mChunkNames as $key => $value)
		{
			...
			{
				lmTextChunk::RegisterChunk($value, $this->mTypes[$key]);
			}
		}
    ...
    public function GetVar($varname)
    {
		...
		// we allow the client of lmResponse to check if a variable is present without validating
		...
		else
		{
			...
			switch ($this->mTypes[$varname])
			{
				case "INT":
					...
					{
						throw new LeapException("Invalid response data for $varname (expected an integer)", LMEC_INVALIDTYPE);
					}
				break;
				...
				case "BIN":
					...
					{
						throw new LeapException("Invalid response data for $varname (expected " . $this->mTypes[$varname] . ")", LMEC_INVALIDTYPE);
					}
				break;
    ...
    private function CheckVarName($varname)
    {
		...
		{
			throw new LeapException("'$varname' is not a valid parameter. Valid parameters are: " . join(", ", array_keys($this->mVariables)), LMEC_INVALIDTYPE);
		}
    }
}