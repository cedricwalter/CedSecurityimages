 ...
 */
class lmPublisherTransaction extends lmTransactionInterface
{
	/**
	...
	*/
	...
	public function GetTransactionID()
	{
		...
		return lmHelper::GetIVFromToken($this->getToken());
	}
	...
	public function __construct($errorCallback, $htmlOnFailure, $useSSL = false, $campaignProfile = null, $templateSelector = null, $purpose = null, $defaultLang = Leap::LANGUAGE_ENGLISH)
	{
		...
		switch ($templateSelector)
		{
			...
			case Leap::TEMPLATE_JAVASCRIPTONLY:
				break;
			default:
				throw new LeapException("Invalid template selector " . $templateSelector, LMEC_INVALIDDATA);
		}
		...
		switch ($purpose)
		{
			...
			case Leap::PURPOSE_GENERIC:
				break;
			default:
				throw new LeapException("Invalid purpose $purpose: " . $purpose, LMEC_INVALIDDATA);
		}
							  ...
							  $tabIndex=null)
	{
		...
		if ($this->getErrorCode() !== false)
		{
			...
			return $begin . $this->GetErrorHTML($this->getErrorCode()) . $end;
		}
	...
	/**
	 * GetErrorHTML:
	 * Given an error code, returns a string of HTML to display to the user
	 *
	 * @param int $errorCode - the error code to put in the email that the user will send to support
	 * @return string
	 */
	...
	public function GetPersistentData()
	{
		...
		{
			return lmTransactionError::getTransactionErrorPersistentData($this->getErrorCode());
		}
