 ...
 *
 **/
class hn_captcha extends JObject  {
  ...
  var $site_tags1;
  var $tag_pos;
  var $watermarkAntiFreePornAttack;
  ...
  /**
   * @shortdesc Absolute URIh to a Tempfolder (with trailing slash!). This must be writeable for PHP and also accessible via HTTP, because the image will be stored there.
   * @type string
   * @public
   *
   **/
  ...
  var $cw_defaultRGBRedBackgroungColor = 224;
  var $cw_defaultRGBGreenBackgroungColor = 224;
  var $cw_defaultRGBBlueBackgroungColor = 224;
  var $cw_useRandomBackgroungColor = TRUE;
  var $cw_minRGBBackgroungColor = 224;
  var $cw_maxRGBBackgroungColor = 255;
  ...
  function hn_captcha($config, $secure = TRUE) {
    ...
    $this->messages = array ('de' => array ('msg1' => 'Du mußt die <b>'.$this->chars.' Zeichen</b> im Bild, (Zahlen&nbsp;von&nbsp;<b>0&nbsp;-&nbsp;9</b> und Buchstaben&nbsp;von&nbsp;<b>A&nbsp;-&nbsp;F</b>),<br>in das Feld eintragen und das Formular abschicken um den Download zu starten.', 'msg2' => 'Ohje, das kann ich nicht lesen. Bitte, generiere mir eine ', 'buttontext' => 'abschicken', 'refreshbuttontext' => 'neue ID'), 'en' => array ('msg1' => 'You must read and type the <b>'.$this->chars.' chars</b> within <b>0..9</b> and <b>A..F</b>, and submit the form.', 'msg2' => 'Oh no, I cannot read this. Please, generate a ', 'buttontext' => 'submit', 'refreshbuttontext' => 'new ID'), 'es' => array ('msg1' => 'Debes leer y teclear los <b>'.$this->chars.' caracteres</b> entre <b>0..9</b> y <b>A..F</b>, y enviar la respuesta.', 'msg2' => 'No puedo leer esto. Por favor, generar un', 'buttontext' => 'Enviar', 'refreshbuttontext' => 'nuevo ID'), 'ru' => array ('msg1' => 'Впишите в �?троку <b>'.$this->chars.' �?имволов</b> �? картинки, �?одержащей цифры от <b>0 до 9</b>, а также <b>латин�?кие</b> буквы <b>от A до F</b>, и нажмите кнопку "Отправить".', 'msg2' => 'Я не могу проче�?ть �?имволы на картинке. Пожалуй�?та, �?генерируйте ', 'buttontext' => 'Отправить', 'refreshbuttontext' => '�?овый �?омер'), 'nl' => array ('msg1' => 'Om te kunnen reageren moet je eerst de <b>'.$this->chars.' karakters</b> tussen <b>0..9</b> en <b>A..F</b> lezen en invoeren. Vervolgens klik je op verzenden.', 'msg2' => 'Getver.., Ik kan dit <b>niet</b> lezen. Genereer a.u.b. een ', 'buttontext' => 'Verzenden', 'refreshbuttontext' => 'Nieuwe code'));
    ...
    $this->public_key = substr(md5(uniqid(mt_rand(), true)), 0, $this->chars);
    if ($this->debug)
  ...
  function display_form() {
    ...
    if ($this->refreshlink) {
      ...
      // Changed by Boriel (www.boriel.com) to work with the captcha! wordpress plugin
      $s .= $this->public_key_input().'<input class="captcha" name="submit" type="submit" value="'.$this->refreshbuttontext.'" />'."</p>\n";
      $s .= '</form>'."\n";
      $s .= '</div>';
    }
    //$s .= '</div>';

    //$s .= '<form action="" method="post"> <!-- NULL form -->';
    $s .= '<form action="'.$_SERVER['PHP_SELF'].$this->QUERY_STRING.'" method="post">';
    $s .= '<input type="hidden" name="dummy" value=""/> <!-- NULL form -->';
    if ($this->debug)
  ...
  function display_captcha($onlyTheImage = FALSE) {
    ...
    if ($onlyTheImage)
    return "\n".'<img class="captchapict" src="'.$this->get_filename_url().'" '.$is[3].' alt="This is a captcha-picture. It is used to prevent mass-access by robots. (see: www.captcha.net)" title="captcha" />'."\n";
    else
  ...
  function make_captcha() {
    ...
    // Set Backgroundcolor
    if ($this->cw_useRandomBackgroungColor) {
      $this->random_color($this->cw_minRGBBackgroungColor, $this->cw_maxRGBBackgroungColor);
      $back = @ imagecolorallocate($image, $this->r, $this->g, $this->b);
    }
    else
    {
      $back = @ imagecolorallocate($image, $this->cw_defaultRGBRedBackgroungColor, $this->cw_defaultRGBGreenBackgroungColor, $this->cw_defaultRGBBlueBackgroungColor);
    }
    ...
    //add by www.waltercedric.com
    if ($this->watermarkAntiFreePornAttack) {
      $site_tags = array ($this->site_tags0, $this->site_tags1);
      $width = $this->lx;
      $height = $this->ly;
  ...
  function get_try($in = TRUE) {
    ...
    if ($in) {
      ...
      for ($i = 1; $i < $this->secretposition; $i ++) {
        // srand((double)microtime()*1000000);
        $a .= $s[intval(mt_rand(1, $this->maxtry))];
      }
      for ($i = 0; $i < (32 - $this->secretposition); $i ++) {
        // srand((double)microtime()*1000000);
        $b .= $s[intval(mt_rand(1, $this->maxtry))];
      }
  ...
  function notvalid_msg() {
    ...
    if ($this->current_try == 1)
    return '&nbsp;<br />&nbsp;';

    // invalid try's: en
    if ($this->lang == "en" && $this->current_try > 2 && $this->refreshlink)
    return 'No valid entry. Please try again:<br />Tip: If you cannot identify the chars, you can generate a new image!';
    if ($this->lang == "en" && $this->current_try >= 2)
    return 'No valid entry. Please try again:<br />&nbsp;';

    // invalid try's: de
    if ($this->lang == "de" && $this->current_try > 2 && $this->refreshlink)
    return 'Die Eingabe war nicht korrekt.<br />Tipp: Wenn Du die Zeichen nicht erkennen kannst, generiere neue mit dem Link unten!';
    if ($this->lang == "de" && $this->current_try >= 2)
    return 'Die Eingabe war nicht korrekt. Bitte noch einmal versuchen:<br />&nbsp;';

    // invalid try's: es
    if ($this->lang == "es" && $this->current_try > 2 && $this->refreshlink)
    return 'Entrada no v&aacute;lida. Prueba otra vez.<br />Consejo: Si no ves bien los caracteres, genera una nueva imagen!';
    if ($this->lang == "es" && $this->current_try >= 2)
    return 'Entrada no v&aacute;lida. Prueba otra vez.';

    // invalid try's: ru
    if ($this->lang == "ru" && $this->current_try > 2 && $this->refreshlink)
    return '�?еправильный код. Пожалуй�?та, попробуйте еще раз.<br />Совет: Е�?ли Вы не можете проче�?ть �?имволы, у Ва�? е�?ть возможно�?ть �?генерировать новую картинку!';
    if ($this->lang == "ru" && $this->current_try >= 2)
    return '�?еправильный код. Пожалуй�?та, попробуйте еще раз.<br />&nbsp;';

    // invalid try's: nl
    if ($this->lang == "nl" && $this->current_try > 2 && $this->refreshlink)
    return 'Geen geldige invoer. Probeer het nog eens a.u.b.:<br />Tip: Als je de karakters niet kunt lezen, kun je een nieuwe afbeelding genereren!';
    if ($this->lang == "nl" && $this->current_try >= 2)
    return 'Geen geldige invoer. Probeer het nog eens a.u.b.:<br />&nbsp;';
  }
